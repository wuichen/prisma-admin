...
const Dashboard = (props) => {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    const { data: customers } = await dataProvider.getMany('customers', {
      ids: aggregations.pendingOrders.map((order: Order) => order.customerId),
    });
    setState((state) => ({
      ...state,
      pendingOrdersCustomers: customers.reduce((prev: CustomerData, customer: Customer) => {
  ...
  const fetchReviews = useCallback(async () => {
    ...
    setState((state) => ({
      ...state,
      pendingReviewsCustomers: customers.reduce((prev: CustomerData, customer: Customer) => {
        prev[customer.id] = customer; // eslint-disable-line no-param-reassign
        return prev;
      }, {}),
    }));
  ...
  useEffect(() => {
    if (permissions && permissions.companyId) {
      fetchOrders();
      fetchReviews();
    }
