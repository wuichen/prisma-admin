...
const ProductPage: NextPage<Props> = ({ deviceType }) => {
  ...
  switch (data.product.type) {
    case 'BOOK': {
      content = <ProductDetailsBook product={data.product} deviceType={deviceType} />;
      break;
    }
    default: {
      content = <ProductDetails product={data.product} deviceType={deviceType} />;
    }
  }

  return (
    <>
      <SEO title={`${data.product.title} - PickBazar`} description={`${data.product.title} Details`} />
