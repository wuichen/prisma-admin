...
export const TextCartItem: React.FC<Props> = ({ data, onDecrement, onIncrement, onRemove }) => {
  const { name, price, salePrice, unit, quantity } = data;
  const displayPrice = salePrice ? salePrice : price;
  // const totalPrice = quantity * displayPrice;
  return (
    <ItemBox>
      <Counter value={quantity} onDecrement={onDecrement} onIncrement={onIncrement} variant="lightVertical" />
      {/* <Image src={image} /> */}
      <Information>
        <Name>{name}</Name>
      </Information>
