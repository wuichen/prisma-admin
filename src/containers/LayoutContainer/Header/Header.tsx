...
const Header: React.FC<Props> = ({ className }) => {
  ...
  const onClickHandler = () => {
    const updatedQuery = query.category ? { text: text, category: query.category } : { text };
    Router.push({
      pathname: pathname,
      query: updatedQuery,
    });
  ...
  return (
    <HeaderWrapper className={className}>
      ...
      <RightMenu isAuthenticated={isAuthenticated} onJoin={handleJoin} onLogout={handleLogout} avatar={UserImage} />
    </HeaderWrapper>
  );
};
