...
const CheckoutWithSidebar: React.FC<MyFormProps> = ({ token, deviceType }) => {
  ...
  // Add or edit modal
  const handleModal = (modalComponent: any, modalProps = {}, className: string = 'add-address-modal') => {
    openModal({
      show: true,
      config: {
        width: 360,
      ...
      <CheckoutWrapper>
        <CheckoutContainer>
          <CheckoutInformation>
            ...
            <InformationBox>
              <Heading>
                <FormattedMessage id="checkoutDeliveryAddress" defaultMessage="Delivery Address" />
              </Heading>
              <ButtonGroup>
                <RadioGroup
                  items={address}
            ...
            <InformationBox className="paymentBox" style={{ paddingBottom: 30 }}>
              ...
              <PaymentGroup
                ...
                items={card}
                onEditDeleteField={(item: any, type: string) => handleEditDelete(item, type, 'payment')}
                onChange={(item: any) =>
                  dispatch({
                ...
                handleAddNewCard={() => {
                  handleModal(StripePaymentForm, { totalPrice: calculatePrice() }, 'add-address-modal stripe-modal');
                }}
              />
              ...
              {coupon ? (
                ...
                <CouponBoxWrapper>
                  {!hasCoupon ? (
                    ...
                    <>
                      <CouponInputBox>
                        <Input onUpdate={handleOnUpdate} value={couponCode} intlPlaceholderId="couponPlaceholder" />
                        <Button onClick={handleApplyCoupon} title="Apply" intlButtonId="voucherApply" />
                      </CouponInputBox>

                      {couponError && (
                        <ErrorMsg>
                          <FormattedMessage id="couponError" defaultMessage={couponError} />
                        </ErrorMsg>
                      )}
                    </>
                  )}
              ...
              <TermConditionText>
                ...
                <Link href="#">
                  <TermConditionLink>
                    <FormattedMessage id="termAndCondition" defaultMessage="terms and conditions." />
                  </TermConditionLink>
                </Link>
              </TermConditionText>
          ...
          <CartWrapper>
            <Sticky enabled={true} top={totalHeight} innerZ={999}>
              <OrderInfo>
                <Title>
                  <FormattedMessage id="cartTitle" defaultMessage="Your Order" />
                </Title>
                ...
                >
                  <ItemsWrapper>
                    {cartItemsCount > 0 ? (
                      items.map((item) => <OrderItem key={`cartItem-${item.id}`} product={item} />)
                    ) : (
                      <NoProductMsg>
                ...
                <CalculationWrapper>
                  ...
                  <TextWrapper style={{ marginTop: 20 }}>
                    <Bold>
                      ...
                      <Small>
                        (
                        <FormattedMessage id="vatText" defaultMessage="Incl. VAT" />)
                      </Small>
                    </Bold>
                    <Bold>
                      {CURRENCY}
