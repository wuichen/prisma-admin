...
const SidebarCategory: React.FC<SidebarCategoryProps> = ({ deviceType: { mobile, tablet, desktop }, type }) => {
  ...
  const handleCategorySelection = (slug: string) => {
    const updatedQuery = state.text ? { text: state.text, category: slug } : { category: slug };
    router.push({
      pathname: pathname,
      query: updatedQuery,
    });
        ...
        <Popover
          ...
          content={
            <>
              ...
              <TreeMenu data={data.categories} onClick={handleCategorySelection} active={selectedQueries} />
            </>
          }
        />
      </PopoverWrapper>
