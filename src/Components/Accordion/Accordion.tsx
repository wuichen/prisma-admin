...
const Accordion: React.FC<AccordionProps> = ({ className, items = [] }) => {
  ...
  return (
    <AccordionWrapper>
      <Collapse accordion={true} className={addAllClasses.join(' ')} defaultActiveKey="active" expandIcon={expandIcon}>
        {items.length !== 0 &&
          items.map((item) => {
            return (
              <Panel
                header={
                  <h3>
                    <FormattedMessage id={item.intlTitleId} defaultMessage="Please add title in language file" />
                  </h3>
                }
