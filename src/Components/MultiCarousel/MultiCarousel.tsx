...
const CarouselWithCustomDots = ({ items = [], deviceType: { mobile, tablet, desktop }, title, ...rest }: any) => {
  ...
  const images = items.map((item: any, index: number) => (
    <Image url={item} key={index} alt={title} style={{ width: '100%', height: '100%', position: 'relative' }} />
  ));
  const CustomDot = ({ index, onClick, active, carouselState: { currentSlide, deviceType } }: any) => {
    return (
      <SingleItem
        data-index={index}
        key={index}
