...
import { CartProduct, CardType, RadioDataType, PaymentOptionType } from './interfaces';
...
export const findProductIndex = (cartProducts: CartProduct[], dataId: number): number => {
  let index = -1;
...
export const getProductQuantity = (cartProducts: CartProduct[], index: number): number => {
  let quantity = 0;
  if (index !== -1) {
...
export const modifyCardData = (data: any, disabled = false, mobileWallet = true, cashOnDelivery = true) => {
