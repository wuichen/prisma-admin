...
const PaymentCard: React.FunctionComponent<PaymentCardProps & any> = ({
  ...
  return (
    <Wrapper htmlFor={`payment-card-${id}`} className={`payment-card-radio ${className ? className : ''}`}>
      <input
        type="radio"
        id={`payment-card-${id}`}
        name={name}
      ...
      <Card id={`card-${id}`} cardType={cardType} lastFourDigit={lastFourDigit} color={color} name={name} />
